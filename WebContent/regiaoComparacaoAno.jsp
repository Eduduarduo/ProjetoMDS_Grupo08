...
<div class="conteudo"> 
<br>
	<p>Selecione anos distintos e uma região para que possamos comparará-las.</p>
	<div style="font-weight: bold; margin: 5px auto;">
		<form id="frm" action="buscaRegiao?cmd=comparacaoAno" method="post">
			Anos: <select name="ano1" id="ano1">
			...
			</select> Região: <select name="regiao" id="regiao">
				<c:forEach var="list" items="${listaRegioes}">
					<option value="${list.regiao}">${list.regiao}</option>
				</c:forEach>
			</select> <input type="button" onclick="changeSerie(this.form);"
				value="Compare" />
		</form>
		<br>
		<br>
